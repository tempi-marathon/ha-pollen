"""EAACI / CAMS-aligned pollen severity levels (grains/m³ → none/low/high).

Boundary convention: a value equal to a threshold belongs to the higher
level (>=). Below onset → none; onset ≤ x < peak → low; >= peak → high.
...
evidence exists (same numbers as the open PollenWatch analytics table for
these six species).
"""
...
def level_for_grains(species: str, grains: float | None) -> int | None:
    """Map grains/m³ to 0/1/2, or None when value/species is unknown."""
    if grains is None:
        return None
